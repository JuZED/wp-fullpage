( function() {
	...
	Backbone.Modal = ( function( _super ) {
		...
		Modal.prototype.prefix = wpfpBbmParams.prefix;
		...
		Modal.prototype.render = function( options ) {
			...
			this.$el.attr( 'id', "" + this.prefix );

			this.modalBackdropEl = Backbone.$( '<div />' ).addClass( "" + this.prefix + "-modal-backdrop" );
			this.modalEl         = Backbone.$( '<div />' ).addClass( "" + this.prefix + "-modal" );
			this.modalContentEl  = Backbone.$( '<div />' ).addClass( "" + this.prefix + "-modal-content" );
			
			this.modalEl.html( this.modalContentEl );
			
			if ( this.template )
				this.modalContentEl.html( this.template( data ) );

			this.$el.html( this.modalEl );
			this.$el.append( this.modalBackdropEl );

			this.modalBackdropEl.on( 'keyup', this.checkKey );
			this.modalBackdropEl.on( 'click', this.clickOutside );
			...
			this.modalEl.addClass( "" + _this.prefix + "-modal--open" )
		...
		Modal.prototype.setUIElements = function() {
			...
			if ( _.isUndefined( this.template ) && _.isUndefined( this.views ) )
				throw new Error( wpfpBbmParams.noTemplateOrViews );

			if ( this.template && this.views && _.isUndefined( this.viewContainer ) )
				throw new Error( wpfpBbmParams.noViewContainer );
		...
		Modal.prototype.clickOutside = function( e ) {
			...
			if ( Backbone.$( e.target ).hasClass( "" + this.prefix + "-modal-backdrop" ) && this.active )
		...
		Modal.prototype.animateToView = function( view ) {
			...
			if ( Backbone.$( 'tester' ).length !== 0 )
				...
				this.modalBackdropEl.append( tester );
		...
		Modal.prototype.close = function() {
			...
			this.modalBackdropEl.off( 'keyup', this.checkKey );
			this.modalBackdropEl.off( 'click', this.clickOutside );
